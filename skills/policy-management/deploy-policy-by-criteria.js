 ...
 */
export async function deployPolicyByCriteria(context, params) {
    try {
        ...
        if (params.identifierType === 'id') {
            ...
            const exactMatch = policies.find((p) => p.name.toLowerCase() === params.policyIdentifier.toLowerCase());
            if (!exactMatch) {
                return {
                    success: false,
                    message: `Policy "${params.policyIdentifier}" not found`,
                    data: { foundPolicies: policies.map((p) => p.name) }
                };
            }
        ...
        // Remove duplicates
        const uniqueDevices = Array.from(new Map(matchingDevices.map(d => [d.id, d])).values());
...
//# sourceMappingURL=deploy-policy-by-criteria.js.map