...
const CATALOG: SearchIndexEntry[] = [
  ...
  { name: 'getComputerApplicationUsage',  signature: '(deviceId: string, startDate: string, endDate: string) => Promise<any>', description: 'Get computer application usage for a YYYY-MM-DD date range', category: 'computers', capabilities: ['read:computers'], readOnly: true },
  { name: 'getClassicApiResource',         signature: '(resourcePath: string) => Promise<any>',                description: 'Read a supported non-sensitive Jamf Classic API resource path as JSON', category: 'classic_api', capabilities: ['read:classic_api'], readOnly: true },
